...
import com.oracle.graal.analysis.mutable.MutableAnalysisType;
...
public class BaseAnalysisType implements MutableAnalysisType, OriginalClassProvider, Comparable<BaseAnalysisType> {

    @SuppressWarnings("rawtypes")//
    private static final AtomicReferenceFieldUpdater<BaseAnalysisType, ConcurrentHashMap> UNSAFE_ACCESS_FIELDS_UPDATER = //
                    AtomicReferenceFieldUpdater.newUpdater(BaseAnalysisType.class, ConcurrentHashMap.class, "unsafeAccessedFields");

    private static final AtomicReferenceFieldUpdater<BaseAnalysisType, ConstantContextSensitiveObject> UNIQUE_CONSTANT_UPDATER = //
                    AtomicReferenceFieldUpdater.newUpdater(BaseAnalysisType.class, ConstantContextSensitiveObject.class, "uniqueConstant");

    @SuppressWarnings("rawtypes")//
    private static final AtomicReferenceFieldUpdater<BaseAnalysisType, Object> INTERCEPTORS_UPDATER = //
                    AtomicReferenceFieldUpdater.newUpdater(BaseAnalysisType.class, Object.class, "interceptors");
    ...
    private static final BaseAnalysisType[] EMPTY_ARRAY = {};

    com.oracle.graal.analysis.domain.AnalysisType[] subTypes;
    com.oracle.graal.analysis.domain.AnalysisType superClass;
     ...
     * finished.
     */
    private List<BaseAnalysisType> referencedTypes;

    /**
     * Cache for the resolved methods.
     *
     * Map ResolvedJavaMethod to Object and not AnalysisMethod because when the type doesn't
     * implement the method the value stored is {@link BaseAnalysisType#NULL_METHOD}.
     */
    private final ConcurrentHashMap<ResolvedJavaMethod, Object> resolvedMethods = new ConcurrentHashMap<>();

    /**
     * Marker used in the {@link BaseAnalysisType#resolvedMethods} map to signal that the type
     * doesn't implement a method.
     */
    private static final Object NULL_METHOD = new Object();

    private final com.oracle.graal.analysis.domain.AnalysisType componentType;
    private final com.oracle.graal.analysis.domain.AnalysisType elementalType;

    private final com.oracle.graal.analysis.domain.AnalysisType[] interfaces;
    ...
    public BaseAnalysisType(AnalysisUniverse universe, ResolvedJavaType javaType, JavaKind storageKind, BaseAnalysisType objectType) {
        ...
        if (isArray()) {
            ...
            int dim = 0;
            BaseAnalysisType elemType = this;
            ...
            for (BaseAnalysisType interf : elemType.getInterfaces()) {
    ...
    public BaseAnalysisType getArrayClass(int dim) {
        BaseAnalysisType result = this;
        for (int i = 0; i < dim; i++) {
     ...
     */
    public List<BaseAnalysisType> getReferencedTypes(BigBang bb) {

        if (referencedTypes == null) {

            Set<BaseAnalysisType> referencedTypesSet = new HashSet<>();

            if (this.isArray()) {
                if (this.getContextInsensitiveAnalysisObject().isObjectArray()) {
                    /* Collect the types referenced through index store (for arrays). */
                    for (BaseAnalysisType type : getContextInsensitiveAnalysisObject().getArrayElementsFlow(bb, false).getState().types()) {
                ...
                for (AnalysisField field : getInstanceFields(true)) {
                    ...
                    for (BaseAnalysisType type : state.types()) {
                        /* Add the assignable types, as discovered by the static analysis. */
                        type.getTypeFlow(bb, false).getState().types().forEach(referencedTypesSet::add);
                    }
                }
    ...
    public static boolean verifyAssignableTypes(StaticAnalysisEngine analysis) {
        // todo points-to specific?
        List<AnalysisType> allTypes = analysis.getUniverse().getTypes();

        boolean pass = true;
        for (AnalysisType t1 : allTypes) {
            BaseAnalysisType baseType = (BaseAnalysisType) t1;
            if (baseType.assignableTypes != null) {
                for (AnalysisType t2 : allTypes) {
                    BaseAnalysisType subtype = (BaseAnalysisType) t2;
                    boolean expected;
                    if (subtype.isInstantiated()) {
                        expected = baseType.isAssignableFrom(subtype);
                    } else {
                        expected = false;
                    }
                    boolean actual = baseType.assignableTypes.getState().containsType(subtype);

                    if (actual != expected) {
                        System.out.println("assignableTypes mismatch: " +
                                        baseType.toJavaName(true) + " (instantiated: " + baseType.isInstantiated() + ") - " +
                                        subtype.toJavaName(true) + " (instantiated: " + subtype.isInstantiated() + "): " +
    ...
    public static void updateAssignableTypes(BigBang bb) {
         ...
         * state.
         */
        List<BaseAnalysisType> allTypes = bb.getUniverse().getTypes().stream().map(it -> ((BaseAnalysisType) it)).collect(Collectors.toList());
        List<TypeFlow<?>> changedFlows = new ArrayList<>();

        Map<Integer, BitSet> newAssignableTypes = new HashMap<>();
        for (BaseAnalysisType type : allTypes) {
            if (type.isInstantiated()) {
                int arrayDimension = type.dimension;
                BaseAnalysisType elementalType = type.elementalType;
    ...
    private static void addTypeToAssignableLists(int typeIdToAdd, BaseAnalysisType elementalType, int arrayDimension, Map<Integer, BitSet> newAssignableTypes, boolean processType) {
        ...
        for (BaseAnalysisType interf : elementalType.getInterfaces()) {
    ...
    private void updateTypeFlows(BigBang bb, TypeFlow<?> assignable, TypeFlow<?> assignableNonNull) {
        ...
        for (BaseAnalysisType type : superAssignableTypeState.types()) {
    ...
    public boolean registerAsReachable() {
        if (!isReachable.get()) {
            ...
            for (BaseAnalysisType iface : interfaces) {
            ...
            if (AtomicUtils.atomicMark(isReachable)) {
                ...
                if (isArray()) {
                    ...
                    for (int i = 1; i <= dimension; i++) {
                        ...
                        for (BaseAnalysisType iface : elementalType.interfaces) {
    ...
    @Override
    public final BaseAnalysisType resolve(ResolvedJavaType accessingClass) {
        return this;
    }
    ...
    @Override
    public final BaseAnalysisType getArrayClass() {
    ...
    public boolean isAssignableFrom(ResolvedJavaType other) {
        ResolvedJavaType subst = universe.substitutions.resolve(((BaseAnalysisType) other).wrapped);
    ...
    @Override
    public BaseAnalysisType getSuperclass() {
        return superClass;
    }

    @Override
    public BaseAnalysisType[] getInterfaces() {
        return interfaces;
    }

    @Override
    public void setSubtypes(Collection<com.oracle.graal.analysis.domain.AnalysisType> subTypes) {
        this.subTypes = subTypes.toArray(new com.oracle.graal.analysis.domain.AnalysisType[subTypes.size()]);
    }
    ...
    @Override
    public BaseAnalysisType findLeastCommonAncestor(ResolvedJavaType otherType) {
        ResolvedJavaType subst = universe.substitutions.resolve(((BaseAnalysisType) otherType).wrapped);
    ...
    @Override
    public BaseAnalysisType getComponentType() {
    ...
    @Override
    public BaseAnalysisType getEnclosingType() {
    ...
    @Override
    public int compareTo(BaseAnalysisType other) {
    ...
    public interface InstanceFieldsInterceptor {
        ResolvedJavaField[] interceptInstanceFields(AnalysisUniverse universe, ResolvedJavaField[] fields, BaseAnalysisType type);
    }
}
